...
class DocumentClassification:
    ...
    def fit(self, data):
        ...
        train_score = self.pipeline.score(
            X_train, self.encode_label(y_train, training=False))
        test_score = self.pipeline.score(
            X_test, self.encode_label(y_test, training=False))
...
class MlPredictor():
    ...
    result = clf.predict_sentence([
        "computer science is a technology.",
        "This music sounds good",
        "math is difficult to learn"]
        )

    >>> ['computer' 'music' 'algebra']
    """

    def __init__(self):
        self.model = DocumentClassification.from_disk()

    def predict_sentence(self, X: List[str]) -> List[str]:
